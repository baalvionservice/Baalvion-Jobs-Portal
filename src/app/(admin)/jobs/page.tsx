...
const JobFormDrawer = dynamic(() => import('@/modules/jobs/components/JobFormDrawer').then(mod => mod.JobFormDrawer), { 
    ssr: false,
    loading: () => (
...
export default function JobsPage() {
    ...
    const [queryParams, setQueryParams] = useState<AdminJobsQueryParams>({ page: 1, limit: 10, search: '' });

    const { data: jobsResponse, isLoading, isError, mutate } = useAdminJobs(queryParams);
    ...
    const handleSaveSuccess = () => {
        ...
        showToast({ type: 'success', title: 'Success', description: `Job has been successfully ${editingJobId ? 'updated' : 'created'}.` });
        mutate();
    }
    ...
    const handleDelete = async (id: string) => {
        try {
            ...
            showToast({ type: 'success', title: 'Job Deleted', description: `Job has been removed.` });
            mutate();
        } catch (error) {
            showToast({ type: 'error', title: 'Error', description: 'Failed to delete job.' });
        }
    };
    ...
    const setFilters = (newFilters: Omit<AdminJobsQueryParams, 'page' | 'limit'>) => {
        ...
        <div className="flex flex-col gap-8">
            <div className="flex items-center justify-between">
                <div>
                    ...
                    <p className="text-muted-foreground">Create, edit, and manage all job postings.</p>
                </div>
            ...
            {isLoading ? (
                ...
                jobsResponse && <JobsTable jobs={jobsResponse.data} departments={jobsResponse.allDepartments} onEdit={handleEdit} onDelete={handleDelete} />
            )}
