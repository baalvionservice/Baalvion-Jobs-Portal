...
import { Briefcase, FilePlus, CalendarCheck, FileText, AlertTriangle, Loader2 } from 'lucide-react';
...
export default function DashboardPage() {
  ...
  if (isError || !data) {
    return (
        <div className="flex flex-col items-center justify-center h-full rounded-lg border-2 border-dashed p-12 text-center">
            ...
            <p className="mt-2 text-sm text-muted-foreground">There was an error fetching the required information. Please try again later.</p>
        </div>
    );
  }
  ...
  return (
    <div className="flex flex-col gap-8">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
            <MetricCard title="Open Positions" value={stats.openJobs} icon={<Briefcase />} description="Active and accepting candidates" />
            <MetricCard title="New Candidates" value={stats.newCandidates} icon={<FilePlus />} description="In the last 7 days" />
            <MetricCard title="Interviews Today" value={stats.interviewsToday} icon={<CalendarCheck />} />
            <MetricCard title="Pending Offers" value={stats.offersPending} icon={<FileText />} />
        </div>
