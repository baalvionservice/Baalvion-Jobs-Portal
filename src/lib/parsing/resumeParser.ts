...
function extractNameAndEmail(text: string): { name: string; email: string, phone: string } {
...
export function parseResumeText(rawText: string): ParsedResumeData | { error: string } {
    ...
    try {
        ...
        if (skills.skills.length === 0 && skills.technologies.length === 0) confidenceScore -= 25;
