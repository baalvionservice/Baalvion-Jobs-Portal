...
import { AnalyticsData, AnalyticsFilters } from '@/modules/analytics/domain/analytics.entity';
import { Candidate, CandidateStage, PaginatedCandidatesResponse } from '@/modules/candidates/candidates.types';
import { Interview, InterviewStatus } from '@/modules/interviews/domain/interview.entity';
import { JobFormData } from '@/modules/jobs/components/creation/schema';
import { AdminJobsQueryParams } from '@/modules/jobs/jobs.hooks';
import { Offer, OfferData, OfferFormData, OfferStatus } from '@/modules/offers/domain/offer.entity';
...
import { Application, ApplicationDetails, ApplicationStatus, ApplicationWithCandidate, AuditLog, CandidateProfileData, Document, DocumentType, Note, Offer as CandidateOffer, Payment, PaymentStatus as CandidatePaymentStatus } from '@/types';
import { AuthUser } from '@/modules/auth/domain/user.types';
import { Organization } from '@/features/organization';
import { Notification } from '@/features/notifications';
import { TeamMember } from '@/lib/team.data';
import { Job as TalentJob, Country, Department, ComplianceProfile } from '@/lib/talent-acquisition';
...
export interface ApiAdapter {
    // Auth
    login(email: string, password: string): Promise<{ success: boolean; userId?: string; message?: string; }>;
    logout(): Promise<void>;
    ...
    // Application
    getApplications(query: TableQuery): Promise<PaginatedResponse<ApplicationWithCandidate>>;
    ...
    submitInterviewFeedback(id: string, feedback: string, rating: number): Promise<void>;

    // Offer
    getAllOffers(): Promise<CandidateOffer[]>;
    deleteOffer(id: string): Promise<void>;
    getOfferForApplication(applicationId: string): Promise<OfferData | null>;
    createOffer(offerData: Partial<OfferFormData> & { applicationId: string, userId: string }): Promise<Offer>;
    updateOfferStatus(offerId: string, status: string, approverId: string): Promise<Offer>;
    getOffersForCandidate(candidateId: string): Promise<CandidateOffer[]>;
    updateCandidateResponse(offerId: string, response: 'ACCEPTED' | 'REJECTED'): Promise<Offer>;

    // Analytics
    getDashboardData(filters: AnalyticsFilters): Promise<AnalyticsData>;

    // Audit
    getAuditLogs(filters: AuditLogFiltersState, limit: number): Promise<{ logs: AuditLog[] }>;
    ...
    uploadDocument(data: { candidateId: string; file: File; documentType: DocumentType; country: string; issueDate?: string; }): Promise<Document>;
    ...
    getTalentDepartments(filters: { isActive?: boolean, countryId?: string }): Promise<Department[]>;
    getTalentJobs(filters: any): Promise<PaginatedResponse<TalentJob>>;
    getTalentJobById(id: string): Promise<TalentJob | undefined>;
    getTalentComplianceProfile(id: string): Promise<ComplianceProfile | undefined>;
    getTalentRolesByCountry(slug: string): Promise<any[]>;

    // College
    getColleges(query: TableQuery): Promise<PaginatedResponse<College & {id: string}>>;
    getAllColleges(): Promise<(College & { id: string; type: "1" | "2" | "3" })[]>;
    ...
    approvePlacement(id: string, updates: { auditLogs: any[] }): Promise<Placement>;
