...
import { PaginatedCandidatesResponse, Candidate, CandidateStage } from "@/modules/candidates/candidates.types";
...
export const candidateMockService = {
    async getCandidates(query: TableQuery): Promise<PaginatedCandidatesResponse> {
        ...
        let filtered = tenantId ? mockCandidates.filter(c => c.tenantId === tenantId) : [];
    ...
    async create(candidateData: Omit<Candidate, 'id' | 'createdAt' | 'tenantId'>): Promise<Candidate> {
        await delay(400);
        const tenantId = localStorage.getItem('talent-os-tenant-id');
    ...
    async getLatestCandidates(limit: number = 5): Promise<Candidate[]> {
        await delay(300);
        const tenantId = localStorage.getItem('talent-os-tenant-id');
        const tenantCandidates = tenantId ? mockCandidates.filter(c => c.tenantId === tenantId) : [];
    ...
    async getCandidateProfile(id: string): Promise<CandidateProfileData | null> {
        ...
        const applicationsForCandidate = mockApplications
            ...
            .map(app => ({
                ...app,
                jobTitle: mockJobs.find(j => j.id === app.jobId)?.title || 'Unknown Job',
            }));
        ...
        return {
            candidate: {
                ...candidate,
                summary: 'Highly motivated software engineer with a passion for building scalable web applications and a proven track record of delivering high-quality code in fast-paced environments.',
                matchScore: 88,
            ...
            stageHistories: mockStageHistories.filter(sh => appIds.has(sh.applicationId)),
