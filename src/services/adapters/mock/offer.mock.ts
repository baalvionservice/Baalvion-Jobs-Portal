...
const tenantOffers = allMockOffers.map((offer, i) => ({
    ...offer,
    status: (offer.applicationId === 'app-2' ? 'SENT' : offer.status) as OfferStatus, // Make one offer 'SENT' for the candidate to act on
...
export const offerMockService = {
    async getAll(): Promise<Offer[]> {
        ...
        const tenantData = tenantId ? statefulOffers.filter(o => o.tenantId === tenantId) : [];

        return tenantData.map(offer => {
            const app = mockApplications.find(a => a.id === offer.applicationId);
            if (!app) return { ...offer, candidateName: 'Unknown', position: 'Unknown' };
    ...
    async getOfferForApplication(applicationId: string): Promise<OfferData | null> {
        await delay(400);
    ...
    async createOffer(offerData: Partial<OfferFormData> & { applicationId: string, userId: string }): Promise<Offer> {
        await delay(600);
        const tenantId = localStorage.getItem('talent-os-tenant-id');
        ...
        const newOffer = {
            ...
            approvals: [
                { approverId: offerData.userId, approverName: 'You', status: 'Approved', timestamp: new Date() },
    ...
    async updateOfferStatus(offerId: string, status: string, approverId: string): Promise<Offer> {
        await delay(400);
        ...
        if (offer && approver) {
            const approval = offer.approvals.find(ap => ap.approverId === approverId);
            if (approval) {
    ...
    async getOffersForCandidate(candidateId: string): Promise<Offer[]> {
        ...
        const candidateApplications = mockApplications.filter(app => app.candidateId === candidateId);
        const candidateAppIds = new Set(candidateApplications.map(app => app.id));

        const candidateOffers = statefulOffers.filter(offer => candidateAppIds.has(offer.applicationId) && offer.status !== 'DRAFT');
    ...
    async updateCandidateResponse(offerId: string, response: 'ACCEPTED' | 'REJECTED'): Promise<Offer> {
        await delay(500);
        const offerIndex = statefulOffers.findIndex(o => o.id === offerId);
        if (offerIndex === -1) throw new Error("Offer not found");
