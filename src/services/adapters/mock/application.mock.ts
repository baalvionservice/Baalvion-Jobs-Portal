...
import { mockApplications as allMockApplications, mockCandidates, mockDocuments, mockInterviews, mockStageHistories } from "@/mocks";
import { mockJobs } from "@/mocks/talent-platform/jobs.mock";
import { Application, ApplicationStatus, Candidate, ApplicationWithCandidate } from "@/types";
import { TableQuery, PaginatedResponse } from "@/components/system/DataTable";

const delay = (ms: number) => new Promise(res => setTimeout(res, ms));

let mockApplications: (Application & { interviewDateTime?: string, offerDate?: string, score?: number | null })[] = [...allMockApplications].map(app => ({
    ...app,
...
export const applicationMockService = {
    ...
    async applyForJob(applicationData: any) {
        ...
        return { success: true, candidateId: newCandidateId, message: "Application submitted successfully!" };
    },
    ...
    async getApplicationsForUser(userId: string) {
        ...
        if (!user) return [];
        const userCandidateIds = mockCandidates.filter(c => c.email === user.email).map(c => c.id);
        return mockApplications.filter(app => userCandidateIds.includes(app.candidateId));
    },
