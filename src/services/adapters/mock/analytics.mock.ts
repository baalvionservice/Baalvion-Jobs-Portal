...
import { AnalyticsData, AnalyticsFilters, PlatformMetrics, MarketplaceHealth } from '@/modules/analytics/domain/analytics.entity';
...
const generateAnalyticsData = (filters: AnalyticsFilters, tenantId: string): AnalyticsData => {
    ...
    const filterByDate = (items: any[]) => {
        ...
        return items.filter(item => isWithinInterval(new Date(item.createdAt), { start: filters.dateRange!.from!, end: filters.dateRange!.to! }));
    };
    ...
    const activeProjects = mockProjects.filter(p => p.status === 'IN_PROGRESS' || p.status === 'ACTIVE').length;
    const completedProjects = mockProjects.filter(p => p.status === 'COMPLETED').length;
    
    const totalEscrowLocked = filteredMilestones
        .filter(m => m.status === 'ACTIVE' || m.status === 'SUBMITTED' || m.status === 'UNDER_REVIEW')
    ...
    const platformCommissionEarned = totalRevenueProcessed * PLATFORM_COMMISSION_RATE;

    const platformKpis: PlatformMetrics = {
        totalUsers: { value: totalUsers, change: 5.2 },
        totalProjects: { value: totalProjects, change: 10.1 },
        activeProjects: { value: activeProjects, change: 3.2 },
        completedProjects: { value: completedProjects, change: 15.6 },
    ...
    const userGrowthTrend = Array.from(userGrowthMap.entries()).map(([date, users]) => ({ date, users }));

    const revenueMap = new Map<string, number>();
    months.forEach(month => revenueMap.set(format(month, 'MMM yyyy'), 0));
    filteredMilestones.filter(m => m.status === 'PAID').forEach(milestone => {
        const monthKey = format(startOfMonth(new Date(milestone.updatedAt)), 'MMM yyyy');
    ...
    const marketplaceHealth: MarketplaceHealth = {
        ...
        avgTimeToCompletion: 28, // days
        fillRate: 92, // percentage
        disputeRate: 1.5, // percentage
    };
